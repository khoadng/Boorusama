...
Parser tagSearchLink(String searchUrl) =>
    (string('{{') & pattern('^}').star().flatten() & string('}}')).map(
      (value) => UrlElement('$searchUrl${value[1]}', displayText: value[1]),
    );

Parser wikiLink(String wikiUrl) =>
    (string('[[') & pattern('^]').star().flatten() & string(']]')).map(
      (value) => UrlElement('$wikiUrl${value[1]}', displayText: value[1]),
    );
