...
class ImagePreloadManager {
  ...
  void preloadImages(PreloadResult result) {
  ...
  void _rebuildPriorityQueue(
    ...
    if (kEnableImagePreloadLogging && _pendingQueue.isNotEmpty) {
      _log(
  ...
  void preloadWithStrategy({
    ...
    required String? Function(int index) urlBuilder,
  }) {
  ...
  ImagePreloadManagerState get state => ImagePreloadManagerState(
    activeDownloads: Set.from(_activeDownloads),
    completedUrls: Set.from(_completedUrls),
  ...
  Future<void> _preloadSingleUrl(String url) async {
    ...
    try {
      ...
      // Let ImageFetcher handle all retries. If we get here, the preload failed
      // after all retries.
      _logState(
        'Download failed',
        url: url,
  ...
  void _logState(
    ...
    if (!kEnableImagePreloadLogging) return;
...
class ImagePreloadManagerState extends Equatable {
  const ImagePreloadManagerState({
