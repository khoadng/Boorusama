...
import '../../../image_preload/types.dart';
import '../../../image_preload/providers.dart';
...
class _PostDetailsImagePreloaderState<T extends Post>
  ...
  ImagePreloadManager? _preloadManager;
  ...
  Future<void> _preloadAdjacentPages(int currentPage) async {
    ...
    _preloadManager?.preloadWithStrategy(
      ...
      urlBuilder: (index) => switch (widget.posts[index]) {
        final post when post.isVideo => post.videoThumbnailUrl,
        ...
        final post => widget.imageUrlBuilder(post),
      },
    );
  }
