...
class _PostDetailsImagePreloaderState<T extends Post>
  ...
  void _preloadPost(T post) {
    ...
    // Don't preload if the URL to preload is the same as original (original logic)
    if (post.originalImageUrl == urlToPreload) {
      return;
    }

    // Avoid duplicate preloads
    if (_preloadedUrls.contains(urlToPreload)) {
      return;
    }
    _preloadedUrls.add(urlToPreload);

    final imagePreloader = ref.read(imagePreloaderProvider(widget.authConfig));
