                  ...
                  children: [
                    ...
                    if (currentTool.isInteractive)
                      ...controller.savedTrackedNotes.map((trackedNote) {
                        final origin = trackedNote.serverId == null
                            ? NoteOrigin.local
                            : NoteOrigin.server;

                        final data = trackedNote.rect.toRectData(
                        ...
                        return LayoutBuilder(
                          builder: (context, noteConstraints) {
                            final note = data.toNote();
                            final widthPercent = widget.image.widthScalePercent(
                              noteConstraints.maxWidth,
                            );
                            final heightPercent = widget.image
                                .heightScalePercent(
                                  noteConstraints.maxHeight,
                                );

                            final scaledCoordinate = note.coordinate
                                .withPercent(widthPercent, heightPercent);

                            if (scaledCoordinate.width <= 0 ||
                                scaledCoordinate.height <= 0) {
