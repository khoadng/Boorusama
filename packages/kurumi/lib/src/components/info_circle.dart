...
enum KurumiInfoCircleStyle { solid, outline }

class KurumiInfoCircleIcon extends StatelessWidget {
  const KurumiInfoCircleIcon({
    super.key,
    this.size = 24.0,
    this.color,
    this.style = KurumiInfoCircleStyle.outline,
    this.semanticLabel,
  });

  final double size;
  final Color? color;
  final KurumiInfoCircleStyle style;
  ...
  Widget build(BuildContext context) {
    ...
    final icon = RepaintBoundary(
      child: CustomPaint(
        size: Size(size, size),
        painter: KurumiInfoCirclePainter(
...
class KurumiInfoCirclePainter extends CustomPainter {
  KurumiInfoCirclePainter({
    required this.color,
    required this.style,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (style == KurumiInfoCircleStyle.solid) {
  ...
  @override
  bool shouldRepaint(KurumiInfoCirclePainter oldDelegate) {
