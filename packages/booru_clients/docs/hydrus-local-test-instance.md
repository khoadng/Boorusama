...
Reuse the existing instance before creating another one:

```sh
docker context ls
docker --context desktop-linux ps -a --filter name=hydrus-client-test
docker --context desktop-linux start hydrus-client-test
```

If the daemon is unavailable, start its engine; an empty container list in a
different Docker context does not mean the test instance is missing.

Only use the following creation command when no test instance exists.
...
Verified with the existing Hydrus 670 / Client API 90 instance: PNG files plus
TXT/JSON sidecars generated by Boorusama's production `SidecarWriter` were
imported through the Hydrus UI, committed, and checked again after restarting
the client.

1. Choose `file -> import files`, add the media files, then
   `add tags/urls with the import -> sidecars`.
2. For TXT, add a TXT-sidecar importer and send its output to the desired local
   tag service. Keep the media extension: `image.png.txt`.
3. For JSON tags, use dictionary key `tags`, then all array items as strings;
   send the result to the local tag service.
4. For JSON URLs, use dictionary key `urls`, then all array items as strings;
   send the result to media URLs. Keep `image.png.json`.
5. Apply/import, then verify `get_files/file_metadata`: the receiving service's
   `storage_tags["0"]` and `known_urls`. Do not rely only on the UI preview.

The check preserved five tags per file, including namespaces, underscores, and
Japanese text; JSON also preserved both source URLs. Hydrus can display
underscores as spaces while retaining the original stored tags.

Do not open the live bind-mounted SQLite database from the host. Perform
inspection inside the container or use the Client API; cross-boundary WAL
access caused a `locking protocol` commit failure during the first attempt.
The rerun used container-only access and passed after a commit and restart.

Remove only the imported test hashes and temporary fixtures after testing.
Preserve the existing container, database, and seed files.
